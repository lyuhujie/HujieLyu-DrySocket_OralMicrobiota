...
### 1.1. 元数据/实验设计 metadata
    ...
    csvtk -t stat result/metadataE_raw.txt
    # 元数据至少3列，首列为样本ID(SampleID)，结尾列为描述(Description)
    # cat查看文件，-A显示符号，"|"为管道符实现命令连用，head显示文件头，-n3控制范围前3行
    cat -A result/metadataE_raw.txt | head -n3
    # windows用户结尾有^M，运行sed命令去除，再用cat -A检查
    sed 's/\r//' result/metadataE_raw.txt > result/metadataE.txt
    cat -A result/metadataE.txt | head -n3
...
### 1.2. 测序数据 sequencing data
    ...
    ls -sh seq/KYSY-11260_KYSY-11259/220256D_320042_1/
    zless seq/KYSY-11260_KYSY-11259/220256D_320042_1/220256D_320042_1_1.fq.gz | head -n4 
    # 每行太长，指定查看每行的1-60个字符
    zless seq/KYSY-11260_KYSY-11259/220256D_320042_1/220256D_320042_1_1.fq | head | cut -c 1-60
    # 统计测序数据，依赖seqkit程序
    seqkit stat seq/KYSY-11260_KYSY-11259/220256D_320042_1/220256D_320042_1_1.fq.gz
    # 批量统计测序数据并汇总表
    seqkit stat seq/KYSY-11260_KYSY-11259/220256D_320042_1/*.fq.gz > result/seqkit.txt
...
### 2.1 合并双端序列并按样品重命名 Merge pair-end reads and rename
    ...
    time for i in `tail -n+2 result/metadata.txt|cut -f1`;do
      vsearch --fastq_mergepairs seq/all12/S26d0_1.fq.gz --reverse seq/all12/S26d0_2.fq.gz \
      --fastqout temp/S26d0.merged.fq --relabel S26d0.
    done &
    
    rename .fq .fastq *.fq 
    rename 's/201030B_//' seq/1030_rawdata/all01/*.fq.gz
    ls seq/KYSY-11260_KYSY-11259/220256D_320042_1/*.fq.gz
    
    
    rename 's/.fastq/.fq/' seq/all12/*.fastq.gz  #将fastq文件变为fq文件
    rename 's/FDMP20H005113-1a_L1_bajd_201030A_//' seq/all12/*.fq.gz
    rename 's/FDMP20H005113-1a_L1_bajd_201030B_//' seq/all12/*.fq.gz
    rename 's/FDMP20H005114-1a_L1_bajd_201030A_//' seq/all12/*.fq.gz
    rename 's/FDMP20H005114-1a_L1_bajd_201030B_//' seq/all12/*.fq.gz
    rename 's/FDMP21H000594-1a_L1_bajd_201030C_//' seq/all12/*.fq.gz
    
    rename 's/FDMP22H010006-1a_L1_220256B_//' seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/*.fq.gz
    rename 's/220256D_//' seq/all12/*.fq.gz
    rename 's/220256C_//' seq/all12/*.fq.gz
    rename 's/_1_R1/_1/' seq/all12/*.fq.gz
    rename 's/_1_R2/_2/' seq/all12/*.fq.gz
    
    rename 's/ts068/S59/' seq/all12/*.fq.gz
    
    mv seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/220256B_E152/*.fq.gz seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/
    mv seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/220256B_E154/*.fq.gz seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/
    mv seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/220256B_E160/*.fq.gz seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/
    mv seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/220256B_E187/*.fq.gz seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/
    mv seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/220256B_E194/*.fq.gz seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/
    mv seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/220256B_E203/*.fq.gz seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/
    mv seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/220256B_E152/*.fq.gz seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/
    mv seq/Result-JD-KYFW-202202-256-JSFW-J037/00.RawData/*.fq.gz seq/all12
    
    i=E203
    vsearch --fastq_mergepairs seq/all12/${i}_1.fq.gz --reverse seq/all12/${i}_2.fq.gz \
      --fastqout temp/${i}.merged.fq --relabel ${i}.
    
    # 批量重命名已有对应列表
    mkdir -p /data5/baidefeng/seg/A/seq
    while read -r line;do
        id=$(echo "$line" | awk '{print $1}')
        sample=$(echo "$line" | awk '{print $2}')
        mv "/mnt/c/16S/temp/$sample.merged.fq" "/mnt/c/16S/temp/$id.merged.fq"       
        # mv "/data5/baidefeng/meta/cancer/meta/A/Ssample/$sample.R2.fq.gz" "/data5/baidefeng/seq/A/sea/Sid.R2.fq.gz"
    done < /mnt/c/16S/temp/metadata_rename.txt
    # 注意将txt文件进行文档格式转换。使用notepad并保存。或者查看网站 https://blog.csdn.net/juejiangjianchi/article/details/82257668 进行脚本修改
    
    
    while read -r line;do
        id=$(echo "$line" | awk '{print $1}')
        sample=$(echo "$line" | awk '{print $2}')
        mv "/data7/lvhujie/oral/seq/${sample}_1.fq.gz" "/data7/lvhujie/oral/seq/${id}_1.fq.gz"       
        mv "/data7/lvhujie/oral/seq/${sample}_2.fq.gz" "/data7/lvhujie/oral/seq/${id}_2.fq.gz"
    done < /data7/lvhujie/oral/metadata_rename.txt
    ...
    #     mv "/data5/baidefeng/meta/cancer/meta/A/Ssample/$sample.R1.fq.gz" "/data5/baidefeng/seq/A/seq/$id.R1.fq.gz"       
    #     mv "/data5/baidefeng/meta/cancer/meta/A/Ssample/$sample.R2.fq.gz" "/data5/baidefeng/seq/A/sea/Sid.R2.fq.gz"
    # done < /data5/baidefeng/meta/metadata01.txt
    # 
    ...
    # time for i in `tail -n+2 result/metadata.txt|cut -f1`;do
    #   vsearch --fastq_mergepairs seq/KYSY-11260_KYSY-11259/220256D_320042_1/*_1.fq.gz --reverse seq/KYSY-11260_KYSY-11259/220256D_320042_1/*_2.fq.gz \
    #    --fastqout temp/test1.merged.fq --relabel test1.
    # done &
    ...
    time tail -n+2 result/metadata.txt | cut -f 1 | \
     rush -j 3 "vsearch --fastq_mergepairs seq/all12/{}_1.fq.gz --reverse seq/all12/{}_2.fq.gz \
    ...
    time tail -n+2 result/metadata.txt | cut -f 1 | \
     rush -j 3 "vsearch --fastq_mergepairs seq/all12/S26d0_1.fq.gz --reverse seq/all12/S26d0_2.fq.gz \
      --fastqout temp/S26d0.merged.fq --relabel S26d0."
    # 检查最后一个文件前10行中样本名
    head temp/`tail -n+2 result/metadata.txt | cut -f 1 | tail -n1`.merged.fq | grep ^@
...
### 2.3 改名后序列整合 integrate renamed reads
    ...
    mkdir -p /c/16S/temp/all_fq
    #合并所有样品至同一文件
    cat temp/*.merged.fq > temp/all_fq/all.fq
    #查看文件大小223M，软件不同版本结果略有差异
    ls -lsh temp/all_fq/all.fq
    ...
    head -n 6 temp/all_fq/all.fq|cut -c1-60
...
## 3. 切除引物与质控 Cut primers and quality filter
    ...
    mkdir -p /c/16S/temp/filtered_fa
    time vsearch --fastx_filter temp/all_fq/all.fq \
      --fastq_stripleft 0 --fastq_stripright 0 \
      --fastq_maxee_rate 0.01 \
      --fastaout temp/filtered_fa/filtered.fa
    # 查看文件了解fa文件格式.因为经过质控后第四行质量值就没用了。
    head temp/filtered_fa/filtered.fa|cut -c1-10
    # head temp/filtered_fa/filtered.fa   也可以

## 4. 去冗余挑选OTU/ASV Dereplicate and cluster/denoise 目的同类的合在一起排序

### 4.1 序列去冗余 Dereplicate

    # 并添加miniuniqusize范围（8-999）1/M，去除低丰度噪音并增加计算速度;输出的特征值在3000-10000.
    # -sizeout输出丰度, --relabel必须加序列前缀更规范, 1s
    mkdir -p /c/16S/temp/uniques_fa
    vsearch --derep_fulllength temp/filtered_fa/filtered.fa \
      --minuniquesize 300 --sizeout --relabel Uni_ \
      --output temp/uniques_fa/uniques.fa 
      
      
    # mkdir -p /c/16S/temp/uniques_fa
    # vsearch --derep_fulllength temp/filtered_fa/filtered.fa \
    #   --minuniquesize 300 --sizeout \
    #   --output temp/filtered_fa/filtered_fa.fa   
    #   
    #   
      
      
    #高丰度非冗余序列非常小(500K~5M较适合)，名称后有size和频率
    ls -lsh temp/uniques_fa/uniques.fa
    # seqkit stat temp/uniques_fa/uniques.fa
    
    # 批量统计测序数据并汇总表
    # seqkit stat seq/*.fq.gz > result/seqkit.txt
    # Uni_1;size=6423  - 去冗余后序列的名字 Uni_1；该序列在所有样品测序数据中出现 6423 次
    # 为出现最多的序列。
    head -n 10 temp/uniques_fa/uniques.fa
    head -n 20 temp/filtered_fa/filtered.fa
...
### 4.2 聚类OTU/去噪ASV Cluster or denoise
    ...
    mkdir -p /c/16S/temp/zotus_fa
    usearch -unoise3 temp/uniques_fa/uniques.fa -minsize 10 \
      -zotus temp/zotus_fa/zotus.fa
    #修改序列名：Zotu为改为ASV方便识别
    sed 's/Zotu/ASV_/g' temp/zotus_fa/zotus.fa > temp/zotus_fa/otus.fa
    head -n 2 temp/zotus_fa/otus.fa
...
### 4.3 基于参考去嵌合 Reference-based chimera detect
    ...
    # 从81行开始数据库下载命令
    vsearch --uchime_ref temp/zotus_fa/otus.fa \
      -db ${db}/usearch/rdp_16s_v18.fa \
      --nonchimeras result/raw/otus_nochimera.fa
    # RDP: 7s, 143 (9.3%) chimeras; SILVA：9m, 151 (8.4%) chimeras
    # Win vsearch结果添加了windows换行符^M需删除，mac不要执行此命令
    sed -i 's/\r//g' result/raw/otus_nochimera.fa
    # head tail -n+10 result/raw/otus.fa
    
    ## 不去的话真实的菌更多些。可以不去
    方法2. 不去嵌合
    cp -f temp/zotus_fa/otus.fa result/raw/otus.fa
...
### 5.1 生成特征表
    ...
    time vsearch --usearch_global temp/filtered_fa/filtered.fa \
      --db result/raw/otus.fa \
      --id 0.97 --threads 4 \
    	--otutabout result/raw/otutab.txt 
    # 上面比对太慢，下面数据少一些快	
    #	uniques_fa/uniques.fa
    	
    time vsearch --usearch_global temp/filtered_fa/filtered_fa.fa \
      --db result/raw/otus.fa \
      --id 0.97 --threads 4 \
    	--otutabout result/raw/otutab.txt 	
...
### 5.2 物种注释，且/或去除质体和非细菌 Remove plastid and non-Bacteria
    ...
    wc -l result/raw/otutab.txt
    #过滤特征表对应序列
    cut -f 1 result/raw/Otutab.txt | tail -n+2 > result/raw/Otutab.id
    usearch -fastx_getseqs result/raw/otus.fa \
        -labels result/raw/Otutab.id -fastaout result/raw/otus.fa
    #过滤特征表对应序列注释
...
### 6.3. 筛选高丰度菌 Filter by abundance-（画进化树有用，一般一棵树100个分支好）
    ...
    awk 'BEGIN{OFS=FS="\t"}{if(FNR==1) {for(i=3;i<=NF;i++) a[i]=$i; print "OTU","Group";} \
        ...
        result/otutab_mean.txt > result/alpha/otu_group_exist（每组中高丰度的菌）.txt
    head result/alpha/otu_group_exist（每组中高丰度的菌）.txt
    cut -f 2 result/alpha/otu_group_exist（每组中高丰度的菌）.txt | sort | uniq -c
...
## 9. 有参定量特征表
    ...
    mkdir -p result/greengene/

    # usearch比对更快，但文件超限报错选附录14 vsearch比对
    # 默认10核以下使用1核，10核以上使用10核
    usearch -otutab temp/filtered.fa -otus ${db}/gg/97_otus.fa \
    	-otutabout result/greengene/otutab.txt -threads 4
    # 比对率80.0%, 1核11m，4核3m，10核2m，内存使用743Mb
    head -n3 result/greengene/otutab.txt

    #统计
    usearch -otutab_stats result/greengene/otutab.txt -output result/greengene/otutab.stat
    cat result/greengene/otutab.stat
...
### 1.3 多样性维恩图
    ...
    bash ${db}/script/sp_vennDiagram.sh \
      -f result/alpha/otu_group_exist（每组中高丰度的菌）.txt \
      -a WT -b KO -c OE \
      -w 3 -u 3 \
      -p WT_KO_OE
...
### 2.3 限制性主坐标分析CPCoA
    ...
    Rscript ${db}/script/beta_cpcoa.R \
      --input result/beta/bray_curtis.txt --design result/metadata.txt \
      --group State --output result/beta/bray_curtis.cpcoa.pdf \
      --width 89 --height 59
    # 添加样本标签 --label TRUE
    Rscript ${db}/script/beta_cpcoa.R \
      --input result/beta/bray_curtis.txt --design result/metadata.txt \
...
## 3. Bugbase细菌表型预测
    ...
    Rscript ${bugbase}/bin/run.bugbase.r -L ${bugbase} \
      -i greengene/otutab.txt -m metadata.txt -c Group -o bugbase/
