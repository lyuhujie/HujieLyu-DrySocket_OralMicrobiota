...
if ("pdf" == "png") {
    png(filename="result/alpha/otu_mean_venn_DHI.txt.D_I.vennDiagram.png", width=3, height=3,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="result/alpha/otu_mean_venn_DHI.txt.D_I.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="result/alpha/otu_mean_venn_DHI.txt.D_I.vennDiagram.pdf", width=3, height=3)
}else if ("pdf" == "svg") {
    svg(filename="result/alpha/otu_mean_venn_DHI.txt.D_I.vennDiagram.svg", width=3, height=3,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    if(num == 5){
        ...
        p <- venn.diagram(
            x = list(D=D, CHENTONG=CHENTONG, I=I,
                     CHENTONG=CHENTONG),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(D=D, I=I, CHENTONG=CHENTONG),
