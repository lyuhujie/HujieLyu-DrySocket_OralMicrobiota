...
if ("pdf" == "png") {
    png(filename="result/alpha/otu_mean_venn_DHI.txt.H_I.vennDiagram.png", width=3, height=3,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="result/alpha/otu_mean_venn_DHI.txt.H_I.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="result/alpha/otu_mean_venn_DHI.txt.H_I.vennDiagram.pdf", width=3, height=3)
}else if ("pdf" == "svg") {
    svg(filename="result/alpha/otu_mean_venn_DHI.txt.H_I.vennDiagram.svg", width=3, height=3,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    data <- read.table(file="result/alpha/otu_mean_venn_DHI.txt", sep="\t", quote="")

    num <- 0

    if("H" != "CHENTONG"){
        H <- data[grepl("\\<H\\>",data[,2]),1]
        num <- num + 1
    }

    if("I" != "CHENTONG"){
        I <- data[grepl("\\<I\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(H=H, CHENTONG=CHENTONG,
                     CHENTONG=CHENTONG, I=I,
                     CHENTONG=CHENTONG),
        ...
        p <- venn.diagram(
            x = list(H=H, CHENTONG=CHENTONG, I=I,
                     CHENTONG=CHENTONG),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(H=H, I=I, CHENTONG=CHENTONG),
