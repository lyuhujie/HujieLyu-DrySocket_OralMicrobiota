...
if ("pdf" == "png") {
    png(filename="result/alpha/otu_mean_venn_premedpost.txt.premedpost1.vennDiagram.png", width=3, height=3,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="result/alpha/otu_mean_venn_premedpost.txt.premedpost1.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="result/alpha/otu_mean_venn_premedpost.txt.premedpost1.vennDiagram.pdf", width=3, height=3)
}else if ("pdf" == "svg") {
    svg(filename="result/alpha/otu_mean_venn_premedpost.txt.premedpost1.vennDiagram.svg", width=3, height=3,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    data <- read.table(file="result/alpha/otu_mean_venn_premedpost.txt", sep="\t", quote="")

    num <- 0

    if("pre" != "CHENTONG"){
        pre <- data[grepl("\\<pre\\>",data[,2]),1]
        num <- num + 1
    }

    if("med" != "CHENTONG"){
        med <- data[grepl("\\<med\\>",data[,2]),1]
        num <- num + 1
    }

    if("post" != "CHENTONG"){
        post <- data[grepl("\\<post\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(pre=pre, CHENTONG=CHENTONG,
                     CHENTONG=CHENTONG, med=med,
                     post=post),
        ...
        p <- venn.diagram(
            x = list(pre=pre, CHENTONG=CHENTONG, med=med,
                     post=post),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(pre=pre, med=med, post=post),
