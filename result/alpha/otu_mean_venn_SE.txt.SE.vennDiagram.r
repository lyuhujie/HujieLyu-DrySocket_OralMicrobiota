...
if ("pdf" == "png") {
    png(filename="result/alpha/otu_mean_venn_SE.txt.SE.vennDiagram.png", width=3, height=3,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="result/alpha/otu_mean_venn_SE.txt.SE.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="result/alpha/otu_mean_venn_SE.txt.SE.vennDiagram.pdf", width=3, height=3)
}else if ("pdf" == "svg") {
    svg(filename="result/alpha/otu_mean_venn_SE.txt.SE.vennDiagram.svg", width=3, height=3,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    data <- read.table(file="result/alpha/otu_mean_venn_SE.txt", sep="\t", quote="")

    num <- 0

    if("S" != "CHENTONG"){
        S <- data[grepl("\\<S\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(S=S, CHENTONG=CHENTONG,
                     CHENTONG=CHENTONG, E=E,
                     CHENTONG=CHENTONG),
        ...
        p <- venn.diagram(
            x = list(S=S, CHENTONG=CHENTONG, E=E,
                     CHENTONG=CHENTONG),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(S=S, E=E, CHENTONG=CHENTONG),
