...
otutab = read.table("C:/16S/result/otutab.txt", header=T, row.names= 1, sep="\t", comment.char = "", stringsAsFactors = F)
# 2. 读取物种注释
tax = read.table("C:/16S/result/taxonomy.txt", header=T, row.names= 1, sep="\t",comment.char = "", stringsAsFactors = F) 

# 数据按实验设计手动筛选(可选)
metadata = read.table("C:/16S/result/metadata_DH.txt", header=T, row.names=1, sep="\t") 
