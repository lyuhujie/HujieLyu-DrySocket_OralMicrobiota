...
md <- read_delim("metadata_all.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
# 样本列表：有些分析只能有2个分组
# getwd()  #显示当前工作目录
# setwd("C:/16S/result")  #更改工作目录.不需要将metadata转移到rmd的工作目录。但是需要将工作目录改回来
# setwd("C:/16S/result/picrust2/out")
...
# PCA
# p = pathway_pca(abundance = kegg_abundance, metadata = md, group = "Group_dayE")
# p = pathway_pca(abundance = kegg_abundance, metadata = md, group = "State")
# ggsave("pathway_pca_state.pdf", p, width = 160, height = 130, units = "mm")
# 筛选前的
# p = pathway_pca(abundance = KEGG_abundance, metadata = md, group = "State")
# ggsave("pathway_pca_state.pdf", p, width = 160, height = 130, units = "mm")
...
tialname = "daa.txt"
tialname2 = "daa.pdf"
result_file_name = "result_all_new"
PCA_names = "pca"
...
annotated_daa
# 进一步过滤 p < 0.05/0.01/0.001的特征.0.005forDSmed-HSmed
...
#                      metadata = md, group = "Group_dayE", reference = "HSmed",
#                      pathway = "HSmed", daa_method = "LinDA", p.adjust = "BH",
#                      #select = "ko04270",
#                      ko_to_kegg = TRUE, order = "pathway_class",
#                      p_values_bar = TRUE, x_lab = "pathway_name")
# 
# # find the statistically significant features with the following command
# daa_results_df %>% filter(p_adjust < 0.01) %>% select(c("feature","p_adjust"))
# 
# result <- ggpicrust2(data = df[,c("function", md$SampleID)],
#                      metadata = md, group = "Group_dayE", reference = "HSmed",
#                      pathway = "HSmed", daa_method = "DESeq2", p.adjust = "none",
#                      ko_to_kegg = TRUE, order = "pathway_class",
#                      p_values_bar = TRUE, x_lab = "pathway_name")
# 
