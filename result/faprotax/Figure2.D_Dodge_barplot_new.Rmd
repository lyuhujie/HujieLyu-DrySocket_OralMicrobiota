...
## 图形绘制 - 均值柱状图
p_mean <- ggplot(mean_data, aes(x = mean_value, y = variable, fill = State)) +
  geom_bar(stat = "identity", position = "dodge") + # 绘制柱状图
  scale_fill_manual(values = custom_colors) + # 应用自定义配色
  labs(title = "Mean OTU Abundance by State") + # 设置标题
  theme_minimal() +
  theme(
    axis.text.y = element_text(size = 6, hjust = 1), # 设置字体大小为 6pt
    axis.text.x = element_text(size = 6), # 设置字体大小为 6pt
  ...
  scale_x_break(c(10, 1000), scales = 0.6) +
  scale_x_continuous(
    breaks = c(0, 2, 4, 6, 8, 10, 1000, 3000, 5000, 7000), # 设置刻度
    labels = c("0", "2", "4", "6","8","10", "1000", "3000", "5000", "7000"), # 设置刻度标签
    expand = c(0, 0), # 防止扩展坐标轴范围
...
ggsave("C:/16S/result/faprotax2/bar_plot_DH_only_new.png", plot = p_mean, width = 6, height = 5)
# Save the plot as a PDF file
ggsave("C:/16S/result/faprotax2/bar_plot_DH_only_new.pdf", plot = p_mean, width = 6, height = 5)
