...
result_file_names <- paste(result_name,State_B,State_C,sep ="_")
sub_metadata_filename <- paste(sub_metadatanames,class_names,Sday_A,State_B,State_C,filetial1,sep ="_")
rfcv_filename_txt <- paste(rfcv_names,class_names,Sday_A,State_B,State_C,filetial1,sep ="_")
rfcv_filename_pdf <- paste(rfcv_names,class_names,Sday_A,State_B,State_C,filetial2,sep ="_")
importance_filename_txt <- paste(importance_names,class_names,Sday_A,State_B,State_C,filetial1,sep ="_")
top_feautre_filename <- paste(top_f,class_names,Sday_A,State_B,State_C,filetial2,sep ="_")
prediction_filename <- paste(prediction_name,class_names,Sday_A,State_B,State_C,filetial1,sep ="_")
testfilename1 <- paste(class_names,test_names,Sday_A,State_B,filetial2,sep ="_")
testfilename2 <- paste(class_names,test_names,Sday_A,State_C,filetial2,sep ="_")
...
sub_metadata1 = subset(metadata_all, State %in% c(State_B,State_C))
sub_metadata1
sub_metadata3 = subset(sub_metadata1, Site_1 %in% c(Site_B))
sub_metadata3
sub_metadata2 = subset(sub_metadata1, Day1 %in% c(Sday_A))
sub_metadata2
...
train = subset(metadata_f, State %in% c("D","I"))
train


# 此处 1:123 需要根据训练集的样本数改变。
idx3 = sample(1:45, size = 24, replace = F)
# 注size=67，66比较小
# 选择的样本标记为TRUE，未选择的为FALSE
idx3 = 1:45 %in% idx3
...
for(i in c("2Phylum","3Class","4Order","5Family","6Genus","8OTU0.02")){
  # i="5Family"
  set.seed(0)
  table_f = read.table(paste0("tax_pre_SE_D_I_",i,".txt"),header = T, row.names = 1)
  table_f = table_f[,rownames(train2)]
  train2$State = as.factor(train2$State)
  rf = randomForest(t(table_f), train2$State, importance=T, proximity=T, ntree = 1000)
  print(i)
  print(rf)
}
...
# train = subset(metadata_f, State %in% c("H","D"))
# train
# train$State = as.factor(train$State)
# 
# # 此处 1:123 需要根据训练集的样本数改变。
# idx3 = sample(1:323, size = 180, replace = F)
# # 注size=67，66比较小
# # 选择的样本标记为TRUE，未选择的为FALSE
# idx3 = 1:323 %in% idx3
# #train2 = metadata_f[idx3,]
...
table =read.table("tax_pre_E_D_I_6Genus.txt",header = T, row.names = 1)
# 筛选L样品作为训练集
...
for (i in 0:9){
  set.seed(i)
  rf2 = randomForest(t(otu_sub1), metadata_train1$State, importance=TRUE, proximity=TRUE, ntree = 1000)
  print(i)
  print(rf2)
}
...
library(randomForest)
set.seed(6)
rf3 = randomForest(t(otu_sub1), metadata_train1$State, importance=TRUE, proximity=TRUE, ntree = 1000)
print(rf3)
```
...
result = rfcv(t(otu_sub1), metadata_train1$State, cv.fold=5)
...
for (i in 1:(1+9)){
  print(i)
  set.seed(i)
  result= rfcv(t(otu_sub1), metadata_train1$State, cv.fold=5) #  scale = "log", step = 0.9
...
otutab_t2$State = metadata_test[rownames(otutab_t2),]$State
```
...
```{r}
# 整理样本原始分组和预测分类
predict = data.frame(group = otutab_t2[,"State"], predicted=otutab.pred)
...
# IND=1, TEJ=2
predict$predict = ifelse(predict$predicted == "D", 1, 2)
# Set sample number in each row
column1 = 4
column = 4

AA1 = predict[predict$group=="I",]$predict
length(AA1)
...
BB1 = predict[predict$group=="D",]$predict
length(BB1)
