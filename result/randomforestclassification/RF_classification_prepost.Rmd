...
test_names = "test"
class_names = "family"
result_name = "resultmed"

result_file_names <- paste(result_name,Sday_A,Sday_C,sep ="_")
sub_metadata_filename <- paste(sub_metadatanames,Sday_A,Sday_C,filetial1,sep ="_")
rfcv_filename_txt <- paste(rfcv_names,Sday_A,Sday_C,filetial1,sep ="_")
rfcv_filename_pdf <- paste(rfcv_names,Sday_A,Sday_C,filetial2,sep ="_")
importance_filename_txt <- paste(importance_names,Sday_A,Sday_C,filetial1,sep ="_")
top_feautre_filename <- paste(top_f,Sday_A,Sday_C,filetial2,sep ="_")
prediction_filename <- paste(prediction_name,Sday_A,Sday_C,filetial1,sep ="_")
testfilename1 <- paste(class_names,test_names,Sday_A,filetial2,sep ="_")
testfilename2 <- paste(class_names,test_names,Sday_B,filetial2,sep ="_")
...
sub_metadata1 = subset(metadata_all, DayE %in% c(Sday_A,Sday_B))
sub_metadata1
...
# test = metadata_f[!idx,]
# 设置训练集
train = subset(metadata_f, DayE %in% c("pre","med"))
train
#train$State = as.factor(train$State)

idx3 = sample(1:268, size = 150, replace = F)
# 210时选class等级,200选Genus水平.premed时150不错，36%；138也行
# 选择的样本标记为TRUE，未选择的为FALSE
idx3 = 1:268 %in% idx3
#train2 = metadata_f[idx3,]
train2 = train[idx3,] # train2是根据实验设计在train里面又筛选的作为训练集的数据。测试集是剩余部分。
train2
train2$DayE = as.factor(train2$DayE)
summary(train2)
...
for(i in c("2Phylum","3Class","4Order","5Family","6Genus","8OTU0.02")){
  # i="5Family"
  set.seed(0)
  table_f = read.table(paste0("tax_pre_med_",i,".txt"),header = T, row.names = 1)
  table_f = table_f[,rownames(train2)]
  train2$DayE = as.factor(train2$DayE)
  rf = randomForest(t(table_f), train2$DayE, importance=T, proximity=T, ntree = 5000)
  print(i)
  print(rf)
}
...
table =read.table("tax_pre_med_4Order.txt",header = T, row.names = 1)
# 筛选L样品作为训练集
...
for (i in 1:10){
  set.seed(i)
  rf2 = randomForest(t(otu_sub1), metadata_train1$DayE, importance=TRUE, proximity=TRUE, ntree = 5000)
  print(i)
  print(rf2)
}
...
```{r}
library(randomForest)
set.seed(5)  # 210时选6,200时选9
rf3 = randomForest(t(otu_sub1), metadata_train1$DayE, importance=TRUE, proximity=TRUE, ntree = 5000)
print(rf3)
```
...
result = rfcv(t(otu_sub1), metadata_train1$DayE, cv.fold=5)
...
for (i in 1:(1+9)){
  print(i)
  set.seed(i)
  result= rfcv(t(otu_sub1), metadata_train1$DayE, cv.fold=5) #  scale = "log", step = 0.9
...
ggsave(top_feautre_filename, p, width=150*2.5, height=59*2, unit='mm') #属水平名字较长，需要扩大宽度。
...
otutab_t2$DayE = metadata_test[rownames(otutab_t2),]$DayE
```
...
pre_tab = table(observed=otutab_t2[,"DayE"],
...
```{r}
# 整理样本原始分组和预测分类
predict = data.frame(group = otutab_t2[,"DayE"], predicted=otutab.pred)
...
# IND=1, TEJ=2
predict$predict = ifelse(predict$predicted == "pre", 1, 2)
# Set sample number in each row
column = 10

AA1 = predict[predict$group=="pre",]$predict
length(AA1)
...
BB1 = predict[predict$group=="med",]$predict
length(BB1)
