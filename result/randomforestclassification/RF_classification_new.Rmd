...
# result_file_names <- paste(result_name,Sday_A,Sday_C,State_A,State_B,sep ="_")
# sub_metadata_filename <- paste(sub_metadatanames,Sday_A,Sday_C,State_A,State_B,filetial1,sep ="_")
# rfcv_filename_txt <- paste(rfcv_names,Sday_A,Sday_C,State_A,State_B,filetial1,sep ="_")
# rfcv_filename_pdf <- paste(rfcv_names,Sday_A,Sday_C,State_A,State_B,filetial2,sep ="_")
# importance_filename_txt <- paste(importance_names,Sday_A,Sday_C,State_A,State_B,filetial1,sep ="_")
# top_feautre_filename <- paste(top_f,Sday_A,Sday_C,State_A,State_B,filetial2,sep ="_")
# prediction_filename <- paste(prediction_name,Sday_A,Sday_C,State_A,State_B,filetial1,sep ="_")
# testfilename1 <- paste(class_names,test_names,Sday_A,Sday_C,State_B,filetial2,sep ="_")
# testfilename2 <- paste(class_names,test_names,Sday_A,Sday_C,State_A,filetial2,sep ="_")

result_file_names <- paste(result_name,State_B,Site_A,Site_B,sep ="_")
sub_metadata_filename <- paste(sub_metadatanames,class_names,State_B,Site_A,Site_B,filetial1,sep ="_")
rfcv_filename_txt <- paste(rfcv_names,class_names,State_B,Site_A,Site_B,filetial1,sep ="_")
rfcv_filename_pdf <- paste(rfcv_names,class_names,State_B,Site_A,Site_B,filetial2,sep ="_")
importance_filename_txt <- paste(importance_names,class_names,State_B,Site_A,Site_B,filetial1,sep ="_")
top_feautre_filename <- paste(top_f,class_names,State_B,Site_A,Site_B,filetial2,sep ="_")
prediction_filename <- paste(prediction_name,class_names,State_B,Site_A,Site_B,filetial1,sep ="_")
testfilename1 <- paste(class_names,test_names,State_B,Site_A,filetial2,sep ="_")
testfilename2 <- paste(class_names,test_names,State_B,Site_B,filetial2,sep ="_")
...
sub_metadata1 = subset(metadata_all, State %in% c(State_B))
sub_metadata1
...
train
train$Site_1 = as.factor(train$Site_1)
...
for(i in c("2Phylum","3Class","4Order","5Family","6Genus","8OTU0.02")){
  # i="5Family"
  set.seed(0)
  table_f = read.table(paste0("tax_D_S_E_",i,".txt"),header = T, row.names = 1)
  table_f = table_f[,rownames(train)]
  train$Site_1 = as.factor(train$Site_1)
  rf = randomForest(t(table_f), train$Site_1, importance=T, proximity=T, ntree = 5000)
  print(i)
  print(rf)
}
...
# train = subset(metadata_f, Group_dayE %in% c("HSpre","DSpre"))
# train
...
# 此处 1:123 需要根据训练集的样本数改变。
idx3 = sample(1:83, size = 42, replace = F)
# 注size=67，66比较小
# 选择的样本标记为TRUE，未选择的为FALSE
idx3 = 1:83 %in% idx3
...
train2
train2$State = as.factor(train2$State)
summary(train2)


# 根据之前筛选情况读取物种分类水平数据。读取科水平特征表
table =read.table("tax_D_S_E_6Genus.txt",header = T, row.names = 1)
# 筛选L样品作为训练集
...
# IND=1, TEJ=2
predict$predict = ifelse(predict$predicted == Site_B, 1, 2)
# Set sample number in each row
column = 5

AA1 = predict[predict$group==Site_B,]$predict
length(AA1)
...
pheatmap(matrix1, cluster_rows = F, cluster_cols = F, cellwidth = 18, cellheight = 18,filename = testfilename2)
# Draw TEJ prediction result
BB1 = predict[predict$group==Site_A,]$predict
length(BB1)
...
pheatmap(matrix2, cluster_rows = F, cluster_cols = F, cellwidth = 18, cellheight = 18, filename = testfilename1)
