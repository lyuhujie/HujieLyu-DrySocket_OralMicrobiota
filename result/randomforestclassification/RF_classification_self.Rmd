...
# 自己设计
idx = sample(1:68, size = 42, replace = F)
# 选择的样本标记为TRUE，未选择的为FALSE
idx = 1:100 %in% idx
...
train$Group_dayE = as.factor(train$Group_dayE)
...
for(i in c("2Phylum","3Class","4Order","5Family","6Genus","8OTU0.02")){
  ...
  set.seed(0)
  table = read.table(paste0("tax_",i,".txt"),header = T, row.names = 1)
  table = table[,rownames(train)]
  train$Group_dayE = as.factor(train$Group_dayE)
  rf = randomForest(t(table), train$Group_dayE, importance=T, proximity=T, ntree = 1000)
  print(i)
  print(rf)
}
...
for (i in 0:9){
  set.seed(i)
  rf = randomForest(t(otu_sub), metadata_train$Group_dayE, importance=TRUE, proximity=TRUE, ntree = 1000)
  print(i)
  print(rf)
}
```
...
result = rfcv(t(otu_sub), metadata_train$Group_dayE, cv.fold=5)
...
for (i in 1:(1+9)){
  print(i)
  set.seed(i)
  result= rfcv(t(otu_sub), metadata_train$Group_dayE, cv.fold=5) #  scale = "log", step = 0.9
...
otutab_t$Group_dayE = metadata[rownames(otutab_t),]$Group_dayE
```
...
pre_tab = table(observed=otutab_t[,"Group_dayE"],
...
```{r}
# 整理样本原始分组和预测分类
predict = data.frame(group = otutab_t[,"Group_dayE"], predicted=otutab.pred)
...
# IND=1, TEJ=2
predict$predict = ifelse(predict$predicted == "DSmed", 1, 2)
# Set sample number in each row
column = 3

DSmed = predict[predict$group=="DSmed",]$predict
length(DSmed)
row = round(length(DSmed)/column + 0.5)
i = column * row - length(DSmed)
DSmed = c(DSmed, rep(NA, i))
matrix = matrix(DSmed, nrow = row, ncol = column, byrow = T)
pheatmap(matrix, cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12)
pheatmap(matrix, cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12,filename = "family_test_DSmed.pdf")

# Draw TEJ prediction result
HSmed = predict[predict$group=="HSmed",]$predict
length(HSmed)
row = round(length(HSmed)/column + 0.5)
i = column * row - length(HSmed)
HSmed = c(HSmed, rep(NA, i))
matrix = matrix(HSmed, nrow = row, ncol = column, byrow = T)
pheatmap(matrix, cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12)
# 保存图片
pheatmap(matrix, cluster_rows = F, cluster_cols = F, cellwidth = 18, cellheight = 18, filename = "family_test_HSmed.pdf")
