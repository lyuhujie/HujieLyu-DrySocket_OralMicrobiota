...
### 1.1 Alpha多样性箱线图
    ...
    # 以上代码用于生成包含I愈合不良组，D干槽症组，H健康组的Alpha多样性箱线图。
    ...
    ## 本代码用于生成正文Figure 1中只包含干槽症和健康组的Alpha多样性箱线图。
    ## 其中使用到的metadata_DH.txt文件可以使用脚本format2stamp.Rmd自主生成。本流程也有提供现成数据。
    ## 下面使用到的区别于metadata.txt的其他原始数据文件均可由format2stamp.Rmd生成。
...
### Figure3 a-f. alpha多样性部分见 C:/16S/All_figures_pipeline/Figure3.A_boxplot+violin.Rmd 脚本      
...
### 曼哈顿图
    ...
    # Figure3.g。对唾液处微生物进行比较。可以更改compare="HSpre-DSpre"分组来展示
    # 中期med和后期post之间的差异。输入"HSmed-DSmed","HSpost-DSpost"来绘制对应图形。
...
### 2.2 绘制扩展柱状图和表
    ...
    # 可以更改compare="HSpre-DSpre"来分析其他时间段。如："HSmed-DSmed" "HSpost-DSpost"
...
### PICRUSt 2.0
    ...
    wc -l KEGG*
    # Figure3.i。根据以上代码生成用于可视化的数据。
    # 可视化见Figure.3i_oral_ggpicrust2.Rmd脚本。
...
## 2. 元素循环FAPROTAX
    ...
    ### 作图可用代码，也可用在线网站。
    ### Figure2.d. 作图代码见 C:/16S/All_figures_pipeline/Figure2.d_Dodge_barplot.Rmd
...
## 4. 进化树可视化
   ## Figure2.a。绘制进化树需要的原始数据，以及设计好的配色方案已经全部输出。使用ITOL网站直接出图。
    # 网址: http://itol.embl.de/ 
   
   
  
# 微生物网络分析
# Figure 4a&b. 需要使用 C:/16S/All_figures_pipeline/Figure4.a_ggclusternet_main_pipeline_new.Rmd以及C:/16S/All_figures_pipeline/Figure4.b_pheatmap_of_network.Rmd
