...
# 所有的文件名称
result_file_names <- paste(result_name,Sday_A,State_A,State_B,"new_Zscore",sep ="_")
...
## tax文件数据处理log10转换，z-score
```{r}
# 获取所有符合命名规则的文件路径
files <- list.files(path = "C:/16S/result/randomforestclassification", pattern = "tax_pre_S_DH_\\d+.*\\.txt$", full.names = TRUE)

# 创建一个函数来处理单个文件
process_file <- function(file_path) {
  # 读取文件
  otu_data <- fread(file_path)
  
  # 获取第一列的列名
  first_col_name <- colnames(otu_data)[1]
  
  # 转换数据框，将第一列设为行名
  otu_data <- otu_data %>%
    column_to_rownames(var = first_col_name)
  
  # 将所有数据都除以 100
  otu_data <- otu_data / 100
  
  # 对数据进行 log10 转换
  otu_data_log10 <- log10(otu_data + 1)  # 为避免 log10(0) 的情况，加 1 进行转换
  
  # 对数据进行 z-score 标准化
  otu_data_zscore <- scale(otu_data_log10)
  
  # 将结果转换回数据框并重新设置第一列为列名
  otu_data_zscore <- as.data.frame(otu_data_zscore) %>%
    rownames_to_column(var = first_col_name)
  
  # 构建新的文件路径
  new_file_path <- sub("(tax_pre_S_DH_\\d+.*)\\.txt$", "\\1_processed.txt", file_path)
  
  # 保存处理后的数据
  write.table(otu_data_zscore, file = new_file_path, sep = "\t", row.names = FALSE, quote = FALSE)
}
...
# 查看处理后的文件
list.files(path = "C:/16S/result/randomforestclassification", pattern = "tax_pre_S_DH_\\d+.*_processed\\.txt$", full.names = TRUE)
```
...
# 设置训练集
train = subset(metadata_f, DayE %in% c("pre"))
train
...
for(i in c("2Phylum","3Class","4Order","5Family","6Genus","8OTU0.02")){
  # i="5Family"
  set.seed(0)
  table_f = read.table(paste0("C:/16S/result/randomforestclassification/","tax_pre_S_DH_",i,"_processed.txt"),header = T, row.names = 1)
...
table =read.table(paste0("C:/16S/result/randomforestclassification/","tax_pre_S_DH_6Genus_processed.txt"),header = T, row.names = 1)
# 筛选L样品作为训练集
...
test = subset(metadata_f, DayE %in% c("med")) 
